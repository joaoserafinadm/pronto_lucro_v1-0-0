...
export default function DfcList(props) {
  ...
  const [pressTimer, setPressTimer] = useState(null); // Para controle do toque longo
      ...
      <div className="col-12">
        <div className="card">
          <div className="card-body">
            ...
            <hr />
            <div className="row">
              <div className="col-12" ref={containerRef} style={{ height: "450px", overflowY: "scroll" }}>
                      ...
                      return (
                        <React.Fragment key={index}>
                          <div
                            ...
                            onTouchStart={() => {
                              const timer = setTimeout(() => setSelectedIndex(index), 600); // 600ms para toque longo
                              setPressTimer(timer);
                            }}
                          ...
                          >
                            <div className="col">
                              <div className="row">
                                <div className="col-12">
                                  <span className="small text-secondary">{formatDate(elem?.paymentDate)}</span>
                                </div>
                              </div>
                              <div className="row mt-1">
                                <div className="col-12">
                                  <span className="fw-bold">{elem?.description ? elem?.description : "Sem descrição"}</span>
                                </div>
                              </div>
                            </div>
                            <div className="d-flex justify-content-end align-items-center" style={{ width: "150px" }}>
                              <span
                                className={`bold text-center text-${
                                  elem?.active === false ? "secondary" : elem?.type === "income" ? "success" : "danger"
                                }`}
                              >
                                {elem?.type === "expense" && "-"}
                                {elem?.type === "income" && "+"}
                          ...
                          {/* Se for o item pressionado, exibe as opções */}
                          {selectedIndex === index && (
